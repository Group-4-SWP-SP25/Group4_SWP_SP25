...
function checkPassword(pass, err) {
    ...
    if (!regexPassword.test(pass.value)) {
        contentError(err, 'The password must be at least 6 characters and must contain both numbers and letters!');
        errorStyle(pass);
        checkValidate = false;
    } else {
        contentError(err, '');
    ...
    if (
        ...
        if (oldPassInput.value === newPassInput.value) {
            ...
            contentError(e_newPass, 'The new password must be different from old password!');
            checkValidate = false;
        } else {
